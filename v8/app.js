...
// ==========================
// CÔNE DE LOCALISATION
// ==========================
const coneAngle = 15;      // ouverture ±15°
const beforeDist = 30;     // 30 m avant
const afterDist = 30;      // 30 m après

// point 30 m avant
const before = destinationPoint(
  obs.lat,
  obs.lon,
  obs.direction,
  obs.distance - beforeDist
);

// point 30 m après
const after = destinationPoint(
  obs.lat,
  obs.lon,
  obs.direction,
  obs.distance + afterDist
);

// coins du cône
const leftBefore = destinationPoint(
  before.lat,
  before.lon,
...
const leftAfter = destinationPoint(
  after.lat,
  after.lon,
  obs.direction - coneAngle,
  afterDist
...
const rightAfter = destinationPoint(
  after.lat,
  after.lon,
  obs.direction + coneAngle,
  afterDist
...
const rightBefore = destinationPoint(
  before.lat,
  before.lon,
  obs.direction + coneAngle,
  beforeDist
...
// dessin du cône
L.polygon([
  [leftBefore.lat, leftBefore.lon],
  [leftAfter.lat, leftAfter.lon],
  [rightAfter.lat, rightAfter.lon],
  [rightBefore.lat, rightBefore.lon]
], {
  color: color,
  fillColor: color,
  fillOpacity: 0.15,
